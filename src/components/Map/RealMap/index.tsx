...
const MapEvents = ({ markers, showBoundaries, highlights }: MapEventsProps) => {
      ...
      if (offlineLayer) {
        ...
        const controlSaveTiles = control.savetiles(offlineLayer, {
          ...
          confirm(layer, successCallback) {
            ...
            if (
              window.confirm(
                `Salvar mapa offline? Serão baixadas ${layer._tilesforSave?.length} imagens.`
              )
            ) {
              successCallback();
            }
          ...
          confirmRemoval(layer, successCallback) {
            // eslint-disable-next-line no-alert
            if (window.confirm('Remover mapa offline?')) {
              successCallback();
            }
          },
  ...
  useEffect(() => {
    if (highlights && isOnline && !networkError) {
      ...
      <MapContainer zoom={initialZoom} tap={false}>
        ...
        {markers?.map((m, index) => (
          <Marker
            ...
            icon={markerIcon(m.color, index.toString())}
          >
            <Popup>
              {markerBody ? (
                markerBody(m)
                ...
                <>
                  <Text weight={'bold'}>{m.descricaoObra}</Text>
                  <Row>
                    <RowItem>
                      <TextDisplay label="Descrição" text={m.descricao} />
                    </RowItem>
                    <RowItem>
                      <TextDisplay label="Horário" text={m.horario} />
                    </RowItem>
                  </Row>
                </>
              )}
